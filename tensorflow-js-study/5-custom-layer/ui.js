...
export function showTestResults (
  ...
  for (let i = 0; i < testExamples; i++) {
    ...
    basePredDiv.className = `pred ${
      baseCorrect ? 'pred-correct' : 'pred-incorrect'
    }`
    customPredDiv.className = `pred ${
      customCorrect ? 'pred-correct' : 'pred-incorrect'
    }`
...
export function plotLosses (lossValues) {
  ...
  lossLabelElement.innerText =
    'base loss: ' +
    lossValues[lossValues.length - 2].loss.toFixed(2) +
    '\ncustom loss: ' +
    lossValues[lossValues.length - 1].loss.toFixed(2)
}
